<script lang="ts">
	...
	let subtext =
		'Please complete the signup process in the opened auth page! This page will reloaded automatically after completion.';
	...
	onMount(() => {
		...
		if (localStorage.getItem('token')) {
			...
			const popupSignupUrl = `${import.meta.env.VITE_AUTH_URL}/signup?client_id=${
			...
			const popup = createPopupWin(popupSignupUrl, 'Sign Up Window', 500, 600);
